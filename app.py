...
def load_and_enrich():
    ...
    company_names = [f"{rng.choice(['Al-','Saudi ','Gulf ','Arabia ','Vision ','National '])}{rng.choice(['Tech','Build','Care','Trade','Energy','Logistics','Finance','Academy','Services','Group'])} {rng.choice(['Co.','Ltd.','Corp.','LLC','Est.'])}" for _ in range(n_companies)]
    company_sectors = rng.choice(sectors, size=n_companies)
    company_regions = rng.choice(regions, size=n_companies)

    # Assign Nitaqat status with realistic distribution
    status_weights = [0.05, 0.10, 0.20, 0.25, 0.25, 0.15]
    ...
    for i, cid in enumerate(company_ids):
        ...
        # Headcount varies by sector
        headcount = int(rng.integers(15, 300))

        # Saudization rate influenced by status
        saudi_base = {
        ...
        saudi_rate = np.clip(saudi_base, 0, 100) / 100

        for _ in range(headcount):
            is_saudi = rng.random() < saudi_rate
            nationality = "Saudi" if is_saudi else rng.choice(
                ["Egyptian", "Pakistani", "Indian", "Yemeni", "Sudanese", "Filipino"], p=[0.25,0.20,0.25,0.15,0.10,0.05]
            )

            gender = rng.choice(genders, p=[0.65, 0.35] if is_saudi else [0.80, 0.20])
            edu    = rng.choice(education_levels, p=[0.10, 0.15, 0.50, 0.18, 0.07])
            age    = rng.choice(age_groups, p=[0.15, 0.35, 0.28, 0.15, 0.07])
            emp    = rng.choice(employment_types, p=[0.65, 0.10, 0.25])
            job    = rng.choice(job_titles)

            # Salary: Saudis earn slightly more on average
            base_salary = {
            ...
            salary_multiplier = (1.15 if is_saudi else 1.0) * rng.uniform(0.7, 1.5)
            salary = int(base_salary * salary_multiplier)

            # Hire date: skewed toward Q1 for Saudis (compliance behaviour)
            year = int(rng.integers(2018, 2024))
            if is_saudi and rng.random() < 0.40:
                month = int(rng.integers(1, 4))   # Jan-Mar spike
            else:
...
with rc1:
    ...
    for i, row in sector_rc.iterrows():
        ...
        fig_radar.add_trace(go.Scatterpolar(
            ...
            fill="toself",
            fillcolor=colors_radar[i % len(colors_radar)] + "0d",
        ))
...
with rc2:
    heatmap_data = sector_rc.set_index("sector")[rc_cols].copy()
...
with g1:
    sector_gap = df[df["sector"].isin(focus_sectors)].groupby("sector").apply(
        lambda g: pd.Series({
...
with g2:
    sal_comp = (
        ...
        .groupby(["sector","nationality"])["monthly_salary_sar"].mean()
        .reset_index()
    )
    sal_saudi    = sal_comp[sal_comp["nationality"]=="Saudi"].rename(columns={"monthly_salary_sar":"Saudi"})
    sal_nonsaudi = (
        sal_comp[sal_comp["nationality"]!="Saudi"]
        .groupby("sector")["monthly_salary_sar"].mean()
        .reset_index()
...
with s1:
    ...
    nonsaudi_monthly = (
        monthly_hires[monthly_hires["nationality"]!="Saudi"]
...
with s2:
    ...
    overqual = df.groupby("sector").apply(
        lambda g: pd.Series({
...
# ── Section 4: Company Risk Scatter ───────────────────────────────────────────
...
fig_scatter = px.scatter(
    ...
    hover_data={
        ...
        "saudization_pct":":.1f","nitaqat_target":":.0f",
        "composite_risk":":.1f","headcount":True,"gap_pct":":.1f",
    },
    color_discrete_map={
...
# ── Section 5: Root Cause Priority Cards ──────────────────────────────────────
...
worst_cause_overall = (
    fco[["rc_salary_uncompetitive","rc_low_recent_hiring","rc_education_mismatch",
         "rc_gender_gap","rc_overreliance_contract"]]
    .mean().idxmax()
    .replace("rc_","").replace("_"," ").title()
)
